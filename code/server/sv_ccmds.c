...
static void SV_Status_f(void) {
	...
	for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++)
	{
		...
		char        botDummyAddress[64];

		if (cl->state == CS_FREE) {
			csPlayer = sv.configstrings[CS_PLAYERS + i];
			if (!csPlayer || !csPlayer[0]) {
				continue;
			}

			playerName = Info_ValueForKey(csPlayer, "name");
			if (!playerName || !playerName[0]) {
				continue;
			}

			Com_Printf("%*u ", colSize[0], i);
			Com_Printf("%*i ", colSize[1], 0);
			Com_Printf("%*u ", colSize[2], 48);

			Com_Printf("%s ", playerName);
			l = SV_Strlen(playerName);
			if (l < colSize[3]) {
				l = colSize[3] - l;
				for (j = 0; j < l; j++) {
					Com_Printf(" ");
			...
			Com_Printf("%*u ", colSize[4], 0);
			Com_sprintf(botDummyAddress, sizeof(botDummyAddress), "10.0.0.%u:%u", i + 10, 12000 + i);
			Com_Printf("%*s ", colSize[5], botDummyAddress);
			Com_Printf("%*u ", colSize[6], 0);
			Com_Printf("%*u", colSize[7], 0);
			Com_Printf("\n");
