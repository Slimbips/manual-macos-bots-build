...
static float UIDMBox_GetChatScaleMultiplier(void)
{
    if (uid.vidHeight >= 1800) {
        return 3.6f;
    }
    if (uid.vidHeight >= 1440) {
        return 3.2f;
    }
    if (uid.vidHeight >= 1200) {
        return 2.3f;
    }
...
str UIDMBox::CalculateBreaks(UIFont *font, str text, float max_width)
{
    ...
    const float textScaleX = getHighResScale()[0] * UIDMBox_GetChatScaleMultiplier();
    const float widthLimit = textScaleX > 0.0f ? (max_width / textScaleX) : max_width;
...
float UIDMBox::PrintWrap(UIFont *font, float x, float y, str text)
{
    ...
    textScale[0] = getHighResScale()[0] * UIDMBox_GetChatScaleMultiplier();
    textScale[1] = getHighResScale()[1] * UIDMBox_GetChatScaleMultiplier();
    ...
    for (;;) {
        ...
        font->Print(x, fY, p1, p2 - p1, textScale);
        p1 = p2 + 1;
        l -= n;
        fY += font->getHeight(textScale);
    }

    if (*p1) {
        font->Print(x, fY, p1, l, textScale);
        fY += font->getHeight(textScale);
    }
