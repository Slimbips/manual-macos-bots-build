...
#define READABLE_FEED_LINES      12
#define READABLE_FEED_LINE_CHARS 256

typedef struct {
    char text[READABLE_FEED_LINE_CHARS];
    int  time;
} readableFeedLine_t;

static readableFeedLine_t s_readableFeed[READABLE_FEED_LINES];
static int                s_readableFeedHead  = 0;
static int                s_readableFeedCount = 0;

static qboolean CG_ReadableFeedShouldSkip(const char *text)
{
    if (!text || !text[0]) {
        return qtrue;
    }
...
void CG_ReadableFeed_Add(const char *rawText)
{
    char line[READABLE_FEED_LINE_CHARS];
    int  len;
    int  index;
    ...
    Q_strncpyz(line, cgi.LV_ConvertString(rawText), sizeof(line));
    ...
    if (!line[0] || CG_ReadableFeedShouldSkip(line)) {
        return;
    }

    if (s_readableFeedCount < READABLE_FEED_LINES) {
        index = (s_readableFeedHead + s_readableFeedCount) % READABLE_FEED_LINES;
        s_readableFeedCount++;
    } else {
        s_readableFeedHead = (s_readableFeedHead + 1) % READABLE_FEED_LINES;
        index              = (s_readableFeedHead + s_readableFeedCount - 1) % READABLE_FEED_LINES;
    }

    Q_strncpyz(s_readableFeed[index].text, line, sizeof(s_readableFeed[index].text));
    s_readableFeed[index].time = cg.time;
}

int CG_ReadableFeed_GetLineCount(void)
{
    return s_readableFeedCount;
}

const char *CG_ReadableFeed_GetLineText(int index)
{
    int slot;

    if (index < 0 || index >= s_readableFeedCount) {
        return "";
    }

    slot = (s_readableFeedHead + index) % READABLE_FEED_LINES;
    return s_readableFeed[slot].text;
}

int CG_ReadableFeed_GetLineTime(int index)
{
    int slot;
    ...
    slot = (s_readableFeedHead + index) % READABLE_FEED_LINES;
    return s_readableFeed[slot].time;
}
...
static void CG_ServerCommand(qboolean modelOnly)
{
    ...
    if (!strcmp(cmd, "print") || !strcmp(cmd, "hudprint")) {
        CG_ReadableFeed_Add(cgi.Argv(1));
        ...
        if (tolower(*type) == 's') {
            CG_ReadableFeed_Add(va("%s %s", victimName, result1));
            cgi.Printf("%c%s %s\n", hudColor, victimName, result1);
        } else if (tolower(*type) == 'p') {
            if (*s2 == 'x') {
                CG_ReadableFeed_Add(va("%s %s %s", victimName, result1, attackerName));
                cgi.Printf("%c%s %s %s\n", hudColor, victimName, result1, attackerName);
            } else {
                CG_ReadableFeed_Add(va("%s %s %s%s", victimName, result1, attackerName, result2));
