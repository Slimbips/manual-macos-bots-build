...
static float CG_GetHudTextScaleMultiplier()
{
    if (cgs.glconfig.vidHeight >= 2160) {
        return 2.0f;
    }
    if (cgs.glconfig.vidHeight >= 1800) {
        return 1.8f;
    }
    if (cgs.glconfig.vidHeight >= 1440) {
        return 1.6f;
    }
    if (cgs.glconfig.vidHeight >= 1200) {
        return 1.35f;
    }
...
static qboolean CG_IsReadableFeedCandidate(const char *text)
{
    if (!text || !text[0]) {
        return qfalse;
    }

    if (strstr(text, "discord") || strstr(text, "Discord") || strstr(text, "http://")
        || strstr(text, "https://") || strstr(text, "www.") || strstr(text, "Thanks to")) {
        return qfalse;
    }
...
static void CG_DrawReadableFeedCenterOverlay(const vec2_t readableFeedScale)
{
    int    i;
    int    totalLines;
    int    visibleCount;
    ...
    if (!cg_readablefeed || !cg_readablefeed->integer) {
        return;
    }

    totalLines   = CG_ReadableFeed_GetLineCount();
    visibleCount = 0;
    lineHeight   = 16.0f * readableFeedScale[1];

    for (i = 0; i < totalLines; ++i) {
        const char *text = CG_ReadableFeed_GetLineText(i);
        int         time = CG_ReadableFeed_GetLineTime(i);

        if (!text[0] || !CG_IsReadableFeedCandidate(text)) {
            continue;
        }

        if (cg.time - time > 12000) {
            continue;
        }
    ...
    y = cgs.glconfig.vidHeight * 0.5f - (visibleCount * lineHeight) * 0.5f;
    ...
    for (i = 0; i < totalLines; ++i) {
        ...
        textWidth = cgi.UI_FontStringWidth(font, text, -1) * readableFeedScale[0];
        x         = cgs.glconfig.vidWidth * 0.5f - textWidth * 0.5f;

        cgi.R_SetColor(shadowColor);
        cgi.R_DrawString(font, text, (x + 2.0f) / readableFeedScale[0], (y + 2.0f) / readableFeedScale[1], -1, readableFeedScale);

        cgi.R_SetColor(color);
        cgi.R_DrawString(font, text, x / readableFeedScale[0], y / readableFeedScale[1], -1, readableFeedScale);
...
void CG_HudDrawElements()
{
    ...
    vec2_t virtualScale;
    vec2_t hudTextScale;
    vec2_t readableFeedScale;
    float  hudTextScaleMultiplier;
    float  readableFeedMultiplier;
    ...
    hudTextScaleMultiplier = CG_GetHudTextScaleMultiplier();
    hudTextScale[0]        = cgs.uiHiResScale[0] * hudTextScaleMultiplier;
    hudTextScale[1]        = cgs.uiHiResScale[1] * hudTextScaleMultiplier;
    readableFeedMultiplier = cg_readablefeed_scale ? cg_readablefeed_scale->value : 1.6f;
    if (readableFeedMultiplier < 1.0f) {
        readableFeedMultiplier = 1.0f;
    } else if (readableFeedMultiplier > 3.0f) {
        readableFeedMultiplier = 3.0f;
    }

    readableFeedScale[0] = cgs.uiHiResScale[0] * readableFeedMultiplier;
    readableFeedScale[1] = cgs.uiHiResScale[1] * readableFeedMultiplier;
    ...
    CG_DrawReadableFeedCenterOverlay(readableFeedScale);
}
