...
void UIConsole::GetConsoleTextScale(vec2_t outScale) const
{
	outScale[0] = getHighResScale()[0];
	outScale[1] = getHighResScale()[1];
}
...
void UIConsole::DrawBottomLine(void)
{
	...
	GetConsoleTextScale(textScale);

	promptwidth = m_font->getWidth(prompt.c_str(), -1);
	top         = m_frame.size.height - m_font->getHeight(textScale);
	topScaled   = top / textScale[1];
	m_font->Print(0.0, topScaled, prompt.c_str(), -1, textScale);
	...
	if (m_refreshcompletionbuffer || m_completionbuffer.length() >= m_currentline.length()) {
		...
		if (widthbeforecaret < iMaxStringWidth) {
			m_font->Print(iXPos, topScaled, m_currentline.c_str(), -1, textScale);
		} else {
			indicatorwidth = m_font->getWidth(indicator, -1);
			m_font->Print(iXPos, topScaled, indicator, -1, textScale);
			...
			if (m_caret > 0) {
				...
				m_font->Print(iXPos, topScaled, pString + 1, -1, textScale);
		...
		m_font->Print(iXPos, topScaled, m_completionbuffer.c_str(), -1, textScale);

		iXPos += completewidth;
		m_font->Print(iXPos, topScaled, &m_currentline[m_completionbuffer.length()], -1, textScale);

		DrawBoxWithSolidBorder(
			UIRect2D(
				iXPos * textScale[0],
				top,
				linewidth * textScale[0],
				m_font->getHeight(textScale)
			),
			UBlack,
			URed,
			1,
	...
	if ((uid.time % 750) > 375 && IsActive()) {
		DrawBox(
			UIRect2D(widthbeforecaret * textScale[0], top, 1.0, m_font->getHeight(textScale)),
			m_foreground_color,
			1.0
		);
	}
...
void UIConsole::CalcLineBreaks(item& theItem)
{
	...
	GetConsoleTextScale(textScale);
	textScaleX = textScale[0] > 0.0f ? textScale[0] : 1.0f;
	maxLineWidth = (m_frame.size.width - m_scroll->getSize().width) / textScaleX;
	...
	for (i = 0; remaining[i]; i++) {
		if (remaining[i] == ' ') {
			...
			if (maxLineWidth < (charlen + lenchecking + lensofar)) {
				if (lensofar) {
...
void UIConsole::Draw(void)
{
	...
	if (m_numitems) {
		...
		top     = (int)(m_frame.size.height - m_font->getHeight(textScale) * (m_scroll->getPageHeight() + 2));
		item    = m_firstitem;
		...
		while (lines_drawn < m_scroll->getPageHeight() + 1) {
			...
			top += m_font->getHeight(textScale);
			lines_drawn++;
			at_line++;
		}
	}
...
void UIDMConsole::GetConsoleTextScale(vec2_t outScale) const
{
	outScale[0] = getHighResScale()[0];
	outScale[1] = getHighResScale()[1];

	float multiplier = 1.0f;
	if (uid.vidHeight >= 1800) {
		multiplier = 3.20f;
	} else if (uid.vidHeight >= 1440) {
		multiplier = 2.80f;
	} else if (uid.vidHeight >= 1200) {
		multiplier = 2.20f;
	}

	outScale[0] *= multiplier;
	outScale[1] *= multiplier;
}
