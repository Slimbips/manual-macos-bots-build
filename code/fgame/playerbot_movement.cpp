...
BotMovement::BotMovement()
{
    ...
    m_iTempAwayTime  = 0;
    m_iNumBlocks     = 0;
    m_iWeaveSwitchTime = 0;
    m_iWeaveDirection  = 1;
    m_fWeaveStrength   = 0.35f;
...
void BotMovement::MoveThink(usercmd_t& botcmd)
{
    ...
    if (level.inttime >= m_iCheckPathTime + 1000 && m_iTempAwayState != 2) {
        ...
        if (m_iTempAwayState && level.inttime >= m_iLastBlockTime + 1000) {
            ...
            if (m_iNumBlocks < 2) {
                ...
                Vector right = Vector(controlledEntity->orientation[1]);
                right.z      = 0;
                if (right.lengthSquared() <= 0.01f) {
                    right = Vector(-forward.y, forward.x, 0);
                }
                VectorNormalize2D(right);

                const float sideDist = 112.0f;
                const float backDist = 176.0f;

                Vector candidates[6];
                candidates[0] = controlledEntity->origin - forward * backDist + right * sideDist;
                candidates[1] = controlledEntity->origin - forward * backDist - right * sideDist;
                candidates[2] = controlledEntity->origin + right * (sideDist + 32.0f);
                candidates[3] = controlledEntity->origin - right * (sideDist + 32.0f);
                candidates[4] = controlledEntity->origin - forward * (backDist + 64.0f);
                candidates[5] = controlledEntity->origin - forward * 96.0f + right * G_CRandom(192.0f);

                bool foundEscape = false;
                for (int i = 0; i < 6; i++) {
                    candidates[i].z = controlledEntity->origin.z;

                    if (!CanMoveTo(candidates[i])) {
                        continue;
                    }

                    m_vCurrentGoal = candidates[i];
                    foundEscape    = true;
                    break;
                }

                if (!foundEscape) {
                    m_vCurrentGoal = controlledEntity->origin - forward * 128.0f + right * G_CRandom(128.0f);
                    m_vCurrentGoal.z = controlledEntity->origin.z;
                }
            }
        }
    ...
    if (!controlledEntity->GetLadder() && m_iTempAwayState == 0) {
        Vector  toGoal;
        float   goalDistSquared;
        qboolean canWeave;

        toGoal            = m_vCurrentGoal - controlledEntity->origin;
        toGoal.z          = 0;
        goalDistSquared   = toGoal.lengthSquared();
        canWeave          = (goalDistSquared >= Square(256)) && (vWishDir.x >= 0.7f);

        if (canWeave) {
            if (!m_iWeaveSwitchTime || level.inttime >= m_iWeaveSwitchTime) {
                m_iWeaveDirection *= -1;
                m_iWeaveSwitchTime = level.inttime + 1000 + (rand() % 501);
                m_fWeaveStrength   = 0.32f + G_Random(0.22f);
            }

            vWishDir.y += (float)m_iWeaveDirection * m_fWeaveStrength;
            vWishDir.y = Q_clamp(vWishDir.y, -1.0f, 1.0f);
        } else {
            m_iWeaveSwitchTime = 0;
            m_iWeaveDirection  = 1;
            m_fWeaveStrength   = 0.35f;
