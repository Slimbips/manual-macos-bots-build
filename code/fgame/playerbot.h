...
private:
    ...
    int    m_iWeaveSwitchTime;
    int    m_iWeaveDirection;
    float  m_fWeaveStrength;
    bool   m_bPathing;
...
public:
    ...
    void          SnapToAngles(Vector vAngles);
...
private:
    ...
    // Human-like behavior
    float m_fHeartRate;              // Simulated stress level (0-100)
    int   m_iLastStressEvent;        // Last time stress changed
    int   m_iMoraleFade;             // Morale recovery timer
    float m_fAimInaccuracy;          // Aiming imperfection (0-1)
    int   m_iLastAimAdjustTime;      // When aim was last re-adjusted
    int   m_iPanicLevel;             // Panic state (0-100)
    bool  m_bReloading;              // Is bot currently reloading
    int   m_iReloadStartTime;        // When reload started
    int   m_iHesitationTime;         // When bot should hesitate
    int   m_iLastCoverCheckTime;     // Last time bot checked for cover
    Vector m_vLastCoverPos;          // Position of last cover
    int   m_iEquipmentUsedTime;      // When bot last used equipment
    float m_fFatigueLevel;           // Fatigue (0-1, higher = more tired)
    int   m_iLastSprintTime;         // When bot last sprinted
    bool  m_bSuppressed;             // Is bot being suppressed by fire
    int   m_iSuppressionEndTime;     // When suppression ends
    int   m_iCommunicationDelay;     // Delay before responding to teammates
    int   m_iLastCommunicationTime;  // When bot last communicated
    int   m_iLeanDirection;          // -1 left, 0 none, 1 right
    int   m_iLeanEndTime;            // When current lean should end
    int   m_iNextLeanDecisionTime;   // When to evaluate a new lean
    int   m_iOrbitDirection;         // -1 clockwise, 1 counter-clockwise
    int   m_iNextOrbitSwitchTime;    // When to switch orbit side
    int   m_iHoldPositionUntilTime;  // Briefly hold a tactical doorway/choke
    int   m_iNextHoldCheckTime;      // Cooldown for checking hold opportunities
    int   m_iCoverPeekState;         // 0 = hidden, 1 = peeking
    int   m_iCoverPeekSwitchTime;    // Next time to toggle peek/hide cycle
...
private:
    ...
    void UpdateStressLevel(void);
    void UpdatePanicLevel(void);
    void UpdateFatigue(void);
    void ApplyAimInaccuracy(Vector& targetPos);
    void CheckForCover(void);
    void UpdateSuppression(void);
    void UpdateReloadBehavior(void);
...
public:
    ...
    void EventDamaged(const Event& ev);
