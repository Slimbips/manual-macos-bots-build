...
static int    nextBotNameShuffleTime = 0;

static const int BOT_NAME_SHUFFLE_INTERVAL_MS = 30 * 60 * 1000;
...
static bool G_IsClientNameInUse(const char *name)
{
    gentity_t   *ent;
    unsigned int n;
...
static void G_ResolveUniqueBotName(char *botName, size_t botNameSize)
{
    char baseName[MAX_NETNAME];
    int  suffix;

    if (!botName || !botNameSize || !*botName) {
        return;
    }

    if (!G_IsClientNameInUse(botName)) {
        return;
    }

    for (unsigned int n = 0; n < (unsigned int)sv_maxbots->integer; n++) {
        cvar_t *v = gi.Cvar_Find(va("g_bot%d_name", n));

        if (!v || !*v->string) {
            continue;
        }

        if (!Q_stricmp(v->string, botName)) {
            continue;
        }

        if (!G_IsClientNameInUse(v->string)) {
            Q_strncpyz(botName, v->string, botNameSize);
            return;
        }
    }

    Q_strncpyz(baseName, botName, sizeof(baseName));

    for (suffix = 2; suffix < 1000; suffix++) {
        Com_sprintf(botName, botNameSize, "%s_%d", baseName, suffix);
        if (!G_IsClientNameInUse(botName)) {
            return;
        }
    }

    Com_sprintf(botName, botNameSize, "bot%d", botId);
}

static bool G_GetUnusedConfiguredBotName(const char *currentName, char *outName, size_t outNameSize)
{
    unsigned int n;
    unsigned int start;
    ...
    start = rand() % sv_maxbots->integer;

    for (n = 0; n < (unsigned int)sv_maxbots->integer; n++) {
        cvar_t      *v;
        unsigned int index = (start + n) % sv_maxbots->integer;

        v = gi.Cvar_Find(va("g_bot%d_name", index));
        if (!v || !*v->string) {
            continue;
        }

        if (currentName && *currentName && !Q_stricmp(v->string, currentName)) {
            continue;
        }
        ...
        Q_strncpyz(outName, v->string, outNameSize);
        return true;
    }
...
static bool G_ShuffleSingleBotName(gentity_t *ent)
{
    ...
    if (!G_IsBot(ent) || !ent->client) {
        return false;
    }

    currentName = ent->client->pers.netname;
    if (!currentName || !*currentName) {
        return false;
    }

    Q_strncpyz(oldName, currentName, sizeof(oldName));

    if (!G_GetUnusedConfiguredBotName(oldName, newName, sizeof(newName))) {
        Com_sprintf(newName, sizeof(newName), "%s_%d", oldName, 2 + (rand() % 98));
        G_ResolveUniqueBotName(newName, sizeof(newName));
    }
    ...
    Q_strncpyz(userinfo, ent->client->pers.userinfo, sizeof(userinfo));
    Info_SetValueForKey(userinfo, "name", newName);

    gi.SetUserinfo(ent - g_entities, userinfo);
    G_ClientUserinfoChanged(ent, userinfo);

    return true;
}

static void G_UpdateBotNameShuffle()
{
    unsigned int numBots;
    unsigned int targetChanges;
    ...
    if (!nextBotNameShuffleTime) {
        nextBotNameShuffleTime = level.inttime + BOT_NAME_SHUFFLE_INTERVAL_MS;
        return;
    }

    if (level.inttime < nextBotNameShuffleTime) {
        return;
    }

    numBots = botManager.getControllerManager().getControllers().NumObjects();
    if (!numBots) {
        nextBotNameShuffleTime = level.inttime + BOT_NAME_SHUFFLE_INTERVAL_MS;
        return;
    }

    targetChanges = numBots / 3;
    if (!targetChanges) {
        targetChanges = 1;
    }

    for (n = 0; n < (unsigned int)game.maxclients && changed < targetChanges; n++) {
        ...
        if ((rand() % 100) < 50 && G_ShuffleSingleBotName(ent)) {
            changed++;
        }
    }
...
gentity_t *G_AddBot(const bot_info_t *info)
{
    ...
    G_ResolveUniqueBotName(botName, sizeof(botName));
...
gentity_t *G_RestoreBot(const saved_bot_t& saved)
{
    ...
    savedName = Info_ValueForKey(saved.userinfo, "name");
    if (savedName && *savedName) {
        Q_strncpyz(botName, savedName, sizeof(botName));
        G_ResolveUniqueBotName(botName, sizeof(botName));
        Info_SetValueForKey(userinfo, "name", botName);
    }
